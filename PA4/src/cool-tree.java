...
abstract class Expression extends TreeNode {
    ...
    public abstract void code(PrintStream s, class_ c);
...
abstract class Case extends TreeNode implements Comparable<Case> {
...
class program extends Program {
    ...
    public static CgenClassTable codegen;
    ...
    */
	...
	codegen = new CgenClassTable(classes, s);
        codegen.code();
...
class class_ extends Class_ {
    ...
    public ArrayList<CgenClassTable.Method> methodList;
    public int index;
    ...
    public void copySymbols(ArrayList<CgenClassTable.Attribute> attrs){
        symTab = new SymbolTable();
        symTab.enterScope();
        
        for(int i=0;i<attrs.size();i++){
            attrs.get(i).index = i+CgenSupport.DEFAULT_OBJFIELDS;
            symTab.addId(attrs.get(i).a.name, attrs.get(i));
        }
    }

    public void setMethodList(ArrayList<CgenClassTable.Method> methodList) {
    ...
    public void codeInit(PrintStream str){
        ...
        //jal to parent constructor
        if(!parent.equals(TreeConstants.No_class))
            CgenSupport.emitJal(parent.getString()+CgenSupport.CLASSINIT_SUFFIX, str);
        
        // add code for expr on each attr
        for (Enumeration e = features.getElements(); e.hasMoreElements();) {
	    Feature f = (Feature)e.nextElement();
            ...
            if(f instanceof attr){
                ...
                if(!(a.init instanceof no_expr)){
                    ...
                    CgenClassTable.Attribute att = (CgenClassTable.Attribute) symTab.lookup(a.name);
                    CgenSupport.emitStore(CgenSupport.ACC, att.index, CgenSupport.SELF, str);
                }
            }
        }
    ...
    public void codeMethods(PrintStream str){
        for (Enumeration e = features.getElements(); e.hasMoreElements();) {
	    Feature f = (Feature)e.nextElement();
    ...
    public void codeMethod(method m, PrintStream str){
        CgenSupport.emitMethodRef(name, m.name, str);
        str.print(CgenSupport.LABEL);
        symTab.enterScope();
        ...
        //Insert parameters into symtab
        int i=2+m.formals.getLength();
        for (Enumeration e = m.formals.getElements(); e.hasMoreElements();) {
            formal fp = (formal) e.nextElement();
            symTab.addId(fp.name, new Integer(i--));
        }
        ...
        CgenSupport.emitLoad(CgenSupport.SELF, 2, CgenSupport.SP, str);
        CgenSupport.emitLoad(CgenSupport.RA, 1, CgenSupport.SP, str);
        CgenSupport.emitAddiu(CgenSupport.SP, CgenSupport.SP, 12+4*m.formals.getLength(), str);
        CgenSupport.emitReturn(str);
        symTab.exitScope();
    }
}
...
class branch extends Case {
    ...
    public int compareTo(Case o) {
        class_ c1 = (class_) program.codegen.probe(type_decl);
        class_ c2 = (class_) program.codegen.probe(((branch)o).type_decl);
...
class assign extends Expression {
    ...
    /** Generates code for this expression.  This method is to be completed 
      ...
      * @param s the output stream 
      * */
    public void code(PrintStream s, class_ c) {
        //TODO: gen code for assing
        expr.code(s, c);
        Object  variable = c.symTab.lookup(name);
        int index;
        String rDest;
        
        if(variable instanceof CgenClassTable.Attribute){
            CgenClassTable.Attribute att =  (CgenClassTable.Attribute)variable;
            index = att.index;
            rDest = CgenSupport.SELF;
        }else{
            index = (Integer)(variable);
            rDest = CgenSupport.FP;
        }
        //TODO: faltan let assing
        
        CgenSupport.emitStore(CgenSupport.ACC, index, rDest, s);
    }
}
...
class static_dispatch extends Expression {
    ...
    /** Generates code for this expression.  This method is to be completed 
      ...
      * */
    public void code(PrintStream s, class_ c) {
        //TODO: gen code for static_dispatch
        //c.symTab.enterScope();
        //add parameters to stack
        class_ eclass = (class_) program.codegen.lookup(type_name);
        int midx = eclass.methodList.indexOf(new CgenClassTable.Method(new method(0, name, null, null, null), null));
        //CgenClassTable.Method m = eclass.methodList.get(midx);
        if(actual.getLength()>0){
            //CgenSupport.emitAddiu(CgenSupport.SP, CgenSupport.SP, -4*actual.getLength(), s);
            //int i = actual.getLength()+1;
            
            for (Enumeration e = actual.getElements()/*, fe = m.m.formals.getElements()*/; e.hasMoreElements();) {
                Expression exp = (Expression) e.nextElement();
                exp.code(s, c);
                CgenSupport.emitStore(CgenSupport.ACC, 0, CgenSupport.SP, s);
                CgenSupport.emitAddiu(CgenSupport.SP, CgenSupport.SP, -4, s);
                //c.symTab.addId(((formal)fe.nextElement()).name, new Integer(i));
            }
        }
        expr.code(s, c);
        CgenSupport.emitCheckVoidObject(method.labelCount, (StringSymbol)c.filename, lineNumber, s);
        method.labelCount++;
        
        CgenSupport.emitLoadAddress(CgenSupport.T1, type_name.getString()+CgenSupport.DISPTAB_SUFFIX, s);
        CgenSupport.emitLoad(CgenSupport.T1, midx, CgenSupport.T1, s);
...
class dispatch extends Expression {
    ...
    /** Generates code for this expression.  This method is to be completed 
      ...
      * */
    public void code(PrintStream s, class_ c) {
        //TODO: gen code for dispatch
        //c.symTab.enterScope();
        AbstractSymbol type = (expr.get_type().equals(TreeConstants.SELF_TYPE)?c.name: expr.get_type());
        //add parameters to stack
        class_ eclass = (class_) program.codegen.lookup(type);
        int midx = eclass.methodList.indexOf(new CgenClassTable.Method(new method(0, name, null, null, null), null));
        //CgenClassTable.Method m = eclass.methodList.get(midx);
        if(actual.getLength()>0){
            //CgenSupport.emitAddiu(CgenSupport.SP, CgenSupport.SP, -4*actual.getLength(), s);
...
class cond extends Expression {
    ...
    /** Generates code for this expression.  This method is to be completed 
      ...
      * */
    public void code(PrintStream s, class_ c) {
        //TODO: gen code for conditional
        int label1=method.labelCount, label2=method.labelCount+1;
        method.labelCount+=2;
        pred.code(s, c);
        CgenSupport.emitLoad(CgenSupport.T1, 3, CgenSupport.ACC, s);
        CgenSupport.emitBeqz(CgenSupport.T1, label1, s);
        then_exp.code(s, c);
        CgenSupport.emitBranch(label2, s);
...
class loop extends Expression {
    ...
    /** Generates code for this expression.  This method is to be completed 
      ...
      * */
    public void code(PrintStream s, class_ c) {
        //TODO: gen code for loop
        int begin=method.labelCount, end=method.labelCount+1;
        method.labelCount+=2;
        CgenSupport.emitLabelDef(begin, s);
        pred.code(s, c);
        CgenSupport.emitLoad(CgenSupport.T1, 3, CgenSupport.ACC, s);
        CgenSupport.emitBeqz(CgenSupport.T1, end, s);
        body.code(s, c);
        CgenSupport.emitBranch(begin, s);
...
class typcase extends Expression {
    ...
    /** Generates code for this expression.  This method is to be completed 
      ...
      * */
    public void code(PrintStream s, class_ c) {
        //TODO: gen code for switch
        int endLabel = method.labelCount++;
        int currentLabel = method.labelCount;
        method.labelCount+= cases.getLength()+1;
        
        expr.code(s, c);
        CgenSupport.emitMove(CgenSupport.T3, CgenSupport.ACC, s);
        CgenSupport.emitStore(CgenSupport.T3, 0, CgenSupport.SP, s);
        CgenSupport.emitAddiu(CgenSupport.SP, CgenSupport.SP, -4, s);
        
        //probar que el objeto no sea void
        CgenSupport.emitBne(CgenSupport.T3, CgenSupport.ZERO, currentLabel, s);
        CgenSupport.emitLoadString(CgenSupport.ACC, (StringSymbol)c.filename, s);
        CgenSupport.emitLoadImm(CgenSupport.T1, lineNumber, s);
        CgenSupport.emitJal("_case_abort2", s);
        
        CgenSupport.emitLabelDef(currentLabel++, s);
        CgenSupport.emitLoad(CgenSupport.T2, 0, CgenSupport.T3, s);
        //ordenar cases
        List<Case> lcases = Collections.list(cases.getElements());
        Collections.sort(lcases);
        for(Case _case : lcases){
            branch b = (branch)_case;
            class_ cl = (class_) program.codegen.probe(b.type_decl);
            int lastIdx = program.codegen.getLastChildIndex(b.type_decl);
            
            CgenSupport.emitBlti(CgenSupport.T2, cl.index, currentLabel, s);
            CgenSupport.emitBgti(CgenSupport.T2, lastIdx, currentLabel, s);
            
            c.symTab.enterScope();
            c.symTab.addId(b.name, -method.fpCount++);
            b.expr.code(s, c);
            c.symTab.exitScope();
            method.fpCount--;
...
class block extends Expression {
    ...
    /** Generates code for this expression.  This method is to be completed 
      ...
      * */
    public void code(PrintStream s, class_ c) {
        //TODO: gen code for block
        for (Enumeration e = body.getElements(); e.hasMoreElements();) {
	    ((Expression)e.nextElement()).code(s, c);
        }
    }
}
...
class let extends Expression {
    ...
    /** Generates code for this expression.  This method is to be completed 
      ...
      * */
    public void code(PrintStream s, class_ c) {
        //TODO: gen code for let
        IntSymbol int0 = (IntSymbol) AbstractTable.inttable.addString("0");
	StringSymbol str0 = (StringSymbol) AbstractTable.stringtable.addString("");
        
        c.symTab.enterScope();
        if(init instanceof no_expr){
            if(type_decl.equals(TreeConstants.Int)){
                CgenSupport.emitLoadInt(CgenSupport.ACC, int0, s);
                ...
                CgenSupport.emitLoadString(CgenSupport.ACC, str0, s);
            }else
                CgenSupport.emitMove(CgenSupport.ACC, CgenSupport.ZERO, s);
        }else
            init.code(s, c);
        CgenSupport.emitStore(CgenSupport.ACC, 0, CgenSupport.SP, s);
        CgenSupport.emitAddiu(CgenSupport.SP, CgenSupport.SP, -4, s);
        c.symTab.addId(identifier, new Integer(-method.fpCount));
        method.fpCount++;
        body.code(s, c);        
        CgenSupport.emitAddiu(CgenSupport.SP, CgenSupport.SP, 4, s);
        method.fpCount--;
...
class lt extends Expression {
    ...
    /** Generates code for this expression.  This method is to be completed 
      ...
      * */
    public void code(PrintStream s, class_ c) {
        //TODO: gen code for lt
        int label = method.labelCount++;
        e1.code(s, c);
        CgenSupport.emitStore(CgenSupport.ACC, 0, CgenSupport.SP, s);
        CgenSupport.emitAddiu(CgenSupport.SP, CgenSupport.SP, -4, s);
        e2.code(s, c);
...
class new_ extends Expression {
    ...
    public void code(PrintStream s, class_ c) {
        ...
        if(type_name.equals(TreeConstants.SELF_TYPE)){
            ...
            CgenSupport.emitJal(CgenSupport.T1, s);                        
        }else{
...
class object extends Expression {
    ...
    public void code(PrintStream s, class_ c) {
        ...
        if(name.equals(TreeConstants.self)){
            ...
            CgenSupport.emitLoad(CgenSupport.ACC, index, rDest, s);
        }
        //TODO: faltan let object
    }
